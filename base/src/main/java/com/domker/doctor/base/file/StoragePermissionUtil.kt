...
object StoragePermissionUtil {
    ...
    fun registerForActivityResult(activity: FragmentActivity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            // Android 11或更高的版本
            android11StoragePermissionLauncher =
            ...
            storagePermissionLauncher =
                activity.registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
                    if (isGranted) {
                        // 此手机是版本低于Android 11，且已获得存储权限
                        resultCallback()
                    } else {
                        // 此手机是版本低于Android 11，且没有存储权限
    ...
    fun requestStoragePermission(activity: FragmentActivity, resultCallback: () -> Unit) {
        StoragePermissionUtil.resultCallback = resultCallback
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
